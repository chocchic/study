...
//	}
	// 로그인 처리 
		...
		@ResponseBody // 응답을 jsp페이지가 아닌, body(ajax 요청한 곳)으로 응답하겠다
		public String ajaxIdAvail(String id) {
			System.out.println("ajaxIdAvail requested");
			int result = memberService.idAvail(id);
			String resStr ="";
			if(result == 1) {
			...
			return resStr; // jsp페이지가 아닌 body로 응답해줄 데이터 전송 (문자열이라 리턴타입도 String)
		}
		*/
		@RequestMapping("ajaxIdAvail")
		public ResponseEntity<String> ajaxIdAvail(String id) {
			...
			// body에 전송할 데이터, 헤더 객체, http 상태코드 (ok = 200, created = 201)
		}
}
