...
public class StreamEx14 {
	public static void main(String[] args) {
		...
		// Person들 중 남자만 필터링해 별도의 List 생성
		//풀어서 생성
		...
		maleList.stream().forEach(l -> System.out.println(l.getName() + " " + l.getGender()));
		
		List<Person> femaleList = list.stream().filter(p-> p.getGender() == Person.Gender.FEMALE).collect(Collectors.toList());
		femaleList.stream().forEach(f -> System.out.println(f.getName() + " " + f.getGender()));
	}
}
