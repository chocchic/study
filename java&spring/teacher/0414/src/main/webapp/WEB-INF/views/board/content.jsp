		...
		<div class="reply_container">
			...
			<sec:authorize access="isAuthenticated()">
				<div class="reply_box">
					...
					<input type="text" placeholder="작성자" value='<sec:authentication property="principal.username"/>' readonly="readonly"/>
		...
		function showList(page){
			...
			replyService.getList({bno:bnoVal, page:page||1}, function(replyCount, data){ // replyCount, data 매개변수 2개로 수정  
				...
				for(let i = 0; i < len; i++){
					...
					// 로그인상태와, 아이디-작성자 비교해서 버튼 띄우기 
					str += "<sec:authorize access='isAuthenticated()'><div class='reply_btns'>";
					if(username != data[i].replyer) {
						str += "<button class='addRepBtn'>답글</button>";
					}
					if(username == data[i].replyer) {
						str += "<button class='modifRepBtn'>수정</button>";
					}
					str += "</div></sec:authorize></div></div>";
				}
