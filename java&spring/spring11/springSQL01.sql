...
select /*+ INDEX_DESC(board pk_board) */ * from board;

select /*+ INDEX_DESC (board pk_board) */ rownum r, b.* from board b;

select /*+ INDEX_DESC(board pk_board) */ rownum r, b.* from board b where rownum > 0 and rownum <= 20;

select rownum, b.* from board b order by bno desc;

select A.* from (select /*+INDEX_DESC(board pk_board) */ rownum r, b.* from board b where rownum <= 10) A where r > 7;