...
### 4) detail.html / update.html파일에 삭제 요청을 위한 링크 추가
* detail은 get으로 날리는 것, update쪽은 post로 ajax처리
```html
<button type="button" class="btn btn-danger removeBtn">삭제</button>
<!--script쪽의 $(".removeBtn")-->
```  
...
### 1) 조회를 할 때 사용하는 PageRequestDTO 클래스에 검색조건과 값을 나타낼 인스턴스 변수를 추가  
```java
	// 검색 항목을 저장할 변수
	private String type;
	// 검색 값을 저장할 변수
...
### 2) MemoServiceImpl클래스에 검색조건을 만들어주는 메서드 생성  
```java
	// 검색 조건을 만들어주는 메서드
