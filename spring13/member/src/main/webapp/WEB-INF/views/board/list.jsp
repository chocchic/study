...
<body>
	...
	<div align="center">
		...
		<c:if test="${pageMaker.prev}">
			<a href="${pageMaker.startPage-1}"style="color : #77878F" class="paging_btn">&lt&nbsp;</a>
		</c:if>
		<%-- 페이지 번호 --%>
		...
		<c:if test="${pageMaker.next}">
			<a href="${pageMaker.endPage+1}"style="color : #77878F" class="paging_btn">&gt</a>
		</c:if>
	</div>
	<br/>
	<div align="center">
		...
		<select name="type">
			...
			>제목만</option>
			<option value="C"
				<c:out value="${pageMaker.cri.type eq 'ㅊ' ? 'selected' : ''}"/>
			>내용만</option>
			<option value="W"
		...
		<input type="text" name="keyword" value = "<c:out value="${pageMaker.cri.keyword eq '' ? '' : pageMaker.cri.keyword}"/>"/>
	...
	<%-- page 번호 누를 때 해당 페이지 요청해줄 숨김 폼태그 --%>
	...
	$(document).ready(function(){
		...
		function checkResult(result) {
			...
			$("a.paging_btn").click(function(e){
				...
				console.log("a 클릭!!"+ e);
				// 폼태그에서 name 속성이 pageNum인 input 태그를 찾아
